function [model] = optimization_fix(X, Y, A, U, corr_2, s, theta0, delta0, iterations)
...
maxLS0 = -likelihood_fix(X, Y, A, U, corr_2, s, theta0, delta0);
dLS=abs(maxLS0);
...
while dLS>1e-3 && i<iterations    %(i can be smaller otherwise the convergence is time-consuming)
    ...
    theta0=fmincon(@(theta) likelihood_fix(X, Y, A, U, corr_2, s, theta, delta0), ...
        theta0, [], [], [], [], [1e-8,1e-8], [5e-3,5e-3], [], options);
    delta0=fmincon(@(delta) likelihood_fix(X, Y, A, U, corr_2, s, theta0, delta), ...
        delta0, [], [], [], [], 1e-4, 1e-1, [], options);
    % calculate likelihood
    maxLS = -likelihood_fix(X, Y, A, U, corr_2, s, theta0, delta0);
    dLS=maxLS-maxLS0; 
    maxLS0=maxLS;
end
