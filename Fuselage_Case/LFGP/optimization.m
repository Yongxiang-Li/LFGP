function [model] = optimization(X, Xt, Y, A, U, D, corr_1, corr_2, s, theta0, phi0, delta0, iterations)
...
maxLS0 = -objective_function(X, Xt, Y, A, U, D, corr_1, corr_2, s, theta0, phi0, delta0);
dLS = abs(maxLS0);
...
while dLS>1e-3 && i<iterations
    ...
    theta0=fmincon(@(theta) objective_function(X, Xt, Y, A, U, D, corr_1, corr_2, s, theta, phi0, delta0), ...
        theta0, [], [], [], [], 1e-7*ones(1,q), 1e-5*ones(1,q), [], options);
    phi0=fmincon(@(phi) objective_function(X, Xt, Y, A, U, D, corr_1, corr_2, s, theta0, phi, delta0), ...
        phi0, [], [], [], [], 1, 500, [], options); 
    delta0=fmincon(@(delta) objective_function(X, Xt, Y, A, U, D, corr_1, corr_2, s, theta0, phi0, delta), ...
        delta0, [], [], [], [], 1e-4, 1, [], options);
    % calculate likelihood
    maxLS = -objective_function(X, Xt, Y, A, U, D, corr_1, corr_2, s, theta0, phi0, delta0);
    dLS = maxLS-maxLS0;
    maxLS0 = maxLS;
end
