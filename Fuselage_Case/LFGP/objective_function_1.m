function z = objective_function_1(X, Xt, Y, A, V, Bb, D, corr_1, corr_2, s, theta, phi, delta)
...
Sigy_inv = eye(ly)/delta-U*omega_inv*U'/delta;
log_Sigy_det = (ly-k)*log(delta) + p*log(Rx_det) + n*log(Sig_det) + log_omega_det;
