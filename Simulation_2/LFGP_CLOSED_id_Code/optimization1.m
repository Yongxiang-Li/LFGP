function [model] = optimization1(X, Xt, Y, Y1, A, U, U1, D, corr_2, s, theta0, delta0, iterations)
...
maxLS0 = -likelihood(X, Xt, Y, Y1, A, U, U1, D, corr_2, s, theta0, delta0);
dLS = abs(maxLS0);
...
while dLS>1e-3 && i<iterations
    ...
    theta0=fmincon(@(theta) likelihood(X, Xt, Y, Y1, A, U, U1, D, corr_2, s, theta, delta0), ...
        theta0, [], [], [], [],8e-8*ones(1,q), 1e-5*ones(1,q), [], options);
    delta0=fmincon(@(delta) likelihood(X, Xt, Y, Y1, A, U, U1, D, corr_2, s, theta0, delta), ...
        delta0, [], [], [], [], 1e-4, 1, [], options);
    % calculate error
    maxLS = -likelihood(X, Xt, Y, Y1, A, U, U1, D, corr_2, s, theta0, delta0);
    dLS = maxLS-maxLS0;
    maxLS0 = maxLS;
end
